...
import { LogIn, LogOut, CalendarClock, Archive, Users, MapPin, RefreshCw } from 'lucide-react';
...
export default function AttendancePage() {
  ...
  const fetchRecords = useCallback(async () => {
    if (!user) return;
    setIsLoading(true);
    const [allRecordsResult, activeSessionResult] = await Promise.all([
        ...
        <CardHeader className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <Button onClick={handleClockOut} variant="destructive" disabled={!canClockOut || isProcessing}>
             ...
             <Button variant="outline" size="icon" onClick={fetchRecords} disabled={isLoading || isProcessing}>
        ...
        <CardContent>
          {isLoading && allRecords.length > 0 ? (
            ...
            <div className="mt-6 border border-dashed rounded-lg p-8 text-center">
              ...
              <p className="mt-4 text-sm text-muted-foreground">No attendance records found in Firestore for any user.</p>
              <p className="text-xs text-muted-foreground">
                Clock in to start the first session.
              </p>
            </div>
          )}
        </CardContent>
