...
export async function getFirestoreBackupData(): Promise<BackupActionResult> {
  try {
    const [servicesResult, productsResult, usersResult, tasksResult, invoicesResult, attendanceResult, categoriesResult, companyProfileResult, customersResult] = await Promise.all([
      getServices(),
      getProducts(),
      getFirebaseUsers(),
      getTasks(),
    ...
    const errors = [servicesResult, productsResult, usersResult, tasksResult, invoicesResult, attendanceResult, categoriesResult, companyProfileResult, customersResult].filter(r => !r.success).map(r => r.message);
    if (errors.length > 0) {
