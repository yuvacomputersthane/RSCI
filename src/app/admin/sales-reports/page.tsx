...
export default function SalesReportPage() {
  ...
  const filteredReportData = useMemo(() => {
    ...
    const invoicesInPeriod = allInvoices.filter(invoice => {
      ...
      return isWithinInterval(invoiceDate, { start: startDate, end: effectiveEndDate });
    });
    ...
    return { dailySales: dailySalesArray, periodTotal, periodInvoiceCount };

  }, [allInvoices, isLoading, selectedRangeType, customStartDate, customEndDate, dateRanges]);
            ...
            <ScrollArea className="h-[450px] rounded-md border print:h-auto print:overflow-visible print:border-none">
              <Table>
                <TableCaption className="print:text-xs print:mt-2">
                  ...
                  {filteredReportData.dailySales.length === 0 && " No sales found for this period."}
                </TableCaption>
                ...
                <TableBody>
                  {filteredReportData.dailySales.length > 0 ? (
                    filteredReportData.dailySales.map((sale) => (
                      <TableRow key={sale.date}>
                        <TableCell className="font-medium">{format(parseISO(sale.date), "PPP")}</TableCell>
                        <TableCell className="text-right">{sale.invoiceCount}</TableCell>
                ...
                {filteredReportData.dailySales.length > 0 && (
                   <TableFooter className="print:bg-transparent">
                      <TableRow className="font-semibold bg-muted/50 hover:bg-muted/60 print:bg-transparent">
                          <TableCell>Total for Period</TableCell>
                          <TableCell className="text-right">{filteredReportData.periodInvoiceCount}</TableCell>
