...
export async function addService(data: ServiceFormData): Promise<AddServiceResult> {
  ...
  const { name, price, categoryId, categoryName } = validationResult.data;
  ...
  try {
    ...
    // Convert timestamp to ISO string before sending to client
...
export async function updateService(serviceId: string, data: ServiceFormData): Promise<ServiceActionResult> {
  ...
  const { categoryId, categoryName, ...rest } = validationResult.data;
  const dataToUpdate = {
    ...rest,
    categoryId: categoryId || null,
    categoryName: categoryName || null
