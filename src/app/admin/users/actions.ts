...
export async function createUserInFirebase(email: string, password: string): Promise<CreateUserResult> {
...
export async function updateUserData(userId: string, data: { fullName?: string; hourlyRate?: number }): Promise<UpdateUserDataResult> {
  if (adminSDKInitializationError) {
    return { success: false, message: adminSDKInitializationError };
  }
  ...
  if (typeof hourlyRate !== 'undefined') {
      if (isNaN(hourlyRate) || hourlyRate < 0) return { success: false, message: "Hourly rate must be a non-negative number."};
