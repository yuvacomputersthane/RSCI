...
export default function InvoicePage({ params }: { params: { id: string } }) {
  ...
  const backLink = isMasterUser ? "/admin" : "/";
    ...
    <div className="bg-secondary/30 min-h-screen p-4 sm:p-8 flex items-start justify-center print:bg-white print:p-0">
      <div className="w-full max-w-4xl space-y-4">
        <div className="flex justify-center gap-4 print:hidden">
            ...
            <Button asChild variant="outline"><Link href={backLink}>Back to Dashboard</Link></Button>
        </div>
            ...
            <CardHeader className="p-6 sm:p-8">
                <div className="flex justify-between items-start flex-wrap gap-4">
                 ...
                 <div className="mt-2 flex justify-between items-start flex-wrap gap-4 text-sm">
                     ...
                     <div className="text-left sm:text-right">
                        {companyProfile?.taxId && <p className="text-muted-foreground">Tax ID: {companyProfile.taxId}</p>}
                     </div>
                 </div>
            </CardHeader>
            <Separator />
