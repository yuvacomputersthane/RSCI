...
import { User, Check, Save, ChevronsUpDown, Calculator, RotateCcw, XCircle, PackageSearch, AlertTriangle, PencilLine, CircleDollarSign, Settings, ShoppingCart, Mail, Phone, Separator as VerticalSeparator } from "lucide-react";
...
export default function BillingForm({ onSubmit, initialData, isSubmitting }: BillingFormProps) {
  ...
  const processAndTransformData = async (formData: BillingFormValues): Promise<TransformedBillingData | null> => {
    if (lineItems.length === 0) {
       toast({
  ...
  const internalFormSubmitHandler = async (dataFromHookForm: BillingFormValues) => {
    const transformedData = await processAndTransformData(dataFromHookForm);
    if (transformedData) {
  ...
  const handleClearForm = () => {
    form.reset({ customerName: "Walk-in Customer", customerEmail: "", customerNumber: "", amount: 0 });
    setLineItems([]);
  ...
  return (
    ...
    <Card className="w-full shadow-lg">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(internalFormSubmitHandler)}>
          <CardContent className="space-y-6">
            {errorLoadingItems && (
              <div className="p-4 text-sm text-destructive-foreground bg-destructive rounded-md flex items-start gap-3">
                <AlertTriangle className="h-5 w-5 mt-0.5 shrink-0" />
                <div>
                  <p className="font-semibold">Failed to load data from database</p>
            ...
            <div className="space-y-4">
               <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="flex items-center"><User className="mr-2 h-4 w-4 text-primary" />Customer Name</FormLabel>
                    <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="flex items-center"><Mail className="mr-2 h-4 w-4 text-primary" />Email (Optional)</FormLabel>
                    <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="flex items-center"><Phone className="mr-2 h-4 w-4 text-primary" />Phone (Optional)</FormLabel>
                    <FormControl>
              ...
              <Popover open={isItemPopoverOpen} onOpenChange={(open) => {
                ...
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      "w-full sm:w-auto justify-between min-w-[200px]",
                      "text-muted-foreground"
                    )}
                    disabled={!!errorLoadingItems}
                  >
          ...
          <CardFooter className="flex justify-between items-center pt-6">
            ...
            <Button type="submit" size="lg" disabled={isSubmitting || lineItems.length === 0}>
